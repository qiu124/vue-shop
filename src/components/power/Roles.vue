<template>
  <div>
      ...
      <el-table :data="rolesList" border stripe>
        <el-table-column label="明细" type="expand">
          <template slot-scope="scope">
            <!-- 渲染一级权限 -->
            <el-row v-for="(item1, i1) in scope.row.children" :key="item1.id" :class="{ dpbuttom: true, dptop: i1 == 0, vcenter: true }">
              <el-col :span="5">
              ...
              <el-col :span="19">
                <!-- 渲染二级权限 -->
                <el-row v-for="(item2, i2) in item1.children" :key="item2.id" :class="{ dptop: i2 != 0, vcenter: true }">
                  <el-col :span="6">
                    <el-tag closable type="success" @close="removeRightById(scope.row, item2.id)"> {{ item1.authName }} </el-tag>
                    <i class="el-icon-caret-right"></i>
                  </el-col>
                  <el-col :span="18">
    ...
    <!-- 编辑角色对话框 -->
    <el-dialog title="添加角色" :visible.sync="editDialogVisible" width="50%" @close="editRoleClose">
    ...
    removeRightById(role, rightId) {
      this.$confirm('确定要删除该权限吗?', '提示', {
        ...
        .then(async () => {
          ...
          //   this.getRolesList() 不建议重新获取所有角色列表，会导致页面更新
